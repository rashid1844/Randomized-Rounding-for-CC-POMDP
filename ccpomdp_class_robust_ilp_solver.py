...
class Node(object):
    def __init__(self, parent=None, parent_idx=0, action_idx=None, safe_post_probability=[],
        ...
        self.risk_u = 0.0  # expected for the whole action
        self.risk_l = 0.0  # expected for the whole action
    ...
    def node_info(self):
        ...
        print('risk', self.risk, self.risk_u)
...
def prune_all_nodes(node_count, terminal_node_count, parent_idx_list, child_list_idx_list, parent_act_idx_list, cc, Min_risk_idx_list, Min_risk_list_idx_list):
    ...
    for idx in range(node_count-1, terminal_start-1, -1):
        if not prune_list[idx] and prune(idx, cc, parent_idx_list, Min_risk_idx_list, Min_risk_list_idx_list, parent_act_idx_list, prune_list, child_list_idx_list, True):
            prune_list[idx] = True
            parent_idx = parent_idx_list[idx]
            while prune(parent_idx, cc, parent_idx_list, Min_risk_idx_list, Min_risk_list_idx_list, parent_act_idx_list, prune_list, child_list_idx_list, False):
            ...
            if parent_idx == 0:
                prune_list[0] = True
                temp_prune_list = list(child_list_idx_list[0][0])
            while len(temp_prune_list) > 0:
                ...
                for obs_list in range(len(child_list_idx_list[temp_idx])):
                    child_list_list += list(child_list_idx_list[temp_idx][obs_list])
                prune_list[np.array(child_list_list, np.int32)] = True
                temp_prune_list += [v for v in child_list_list if v < terminal_start]
...
def prune(idx, cc, parent_idx_list, Min_risk_idx_list, Min_risk_list_idx_list, parent_act_idx_list, prune_list, child_list_idx_list, terminal_bool):
    ...
    if not terminal_bool:
        for obs in range(len(child_list_idx_list[idx])):
            if 0 == np.sum(np.array([1 for act_idx in child_list_idx_list[idx][obs] if not prune_list[act_idx]], np.int32)):
                return True

    prev_parent = idx
    parent = parent_idx_list[idx]
    risk_sum = Min_risk_idx_list[idx]
    while parent != 0:
        ...
        prev_parent = parent_idx_list[prev_parent]
        parent = parent_idx_list[parent]
    return risk_sum > cc
...
def calc_min_risk(node_count, terminal_node_count,risk_list, child_list_idx_list, obs_count):
    ...
    for idx in range(node_count-1, -1, -1):
        ...
        if idx < node_count - terminal_node_count:
            Min_risk_list = np.zeros(len(child_list_idx_list[idx]), dtype=np.float64)
            for obs_idx in range(len(child_list_idx_list[idx])):
                Min_risk_list[obs_idx] = np.min(np.array([Min_risk_idx_list[child_idx] for child_idx in child_list_idx_list[idx][obs_idx]], np.float64))
            Min_risk += np.sum(Min_risk_list)
            Min_risk_list_idx_list[idx] = Min_risk_list
        Min_risk_idx_list[idx] = Min_risk
...
class CCPOMDP_robust:
    def __init__(self, initial_belief, model=None, risk_error_range=0.1, risk_error_probability=0.3 ,prune=False, rounding_min_iteration=0, greedy_iteration=0, greedy_advanced=False, robust=True):  # action_list, trans_prob, obs_prob, t_horizon, s_horizon, reward_func, risk_func,duration_func, cc, reset_actions):
        self.initial_belief = initial_belief  # np.array
        self.model = model

        self.robust = robust
        self.risk_error_range = risk_error_range
        self.risk_error_probability = risk_error_probability
    ...
    def del_prune_list(self, prune_list, node_count, terminal_node_count):
        ...
        for depth in range(1, len(self.depth_idx_list)-1):
            self.depth_idx_list[depth] -= np.sum((self.depth_idx_list[depth] < prune_list) & (prune_list < self.depth_idx_list[depth+1]))
    ...
    def expand_tree(self):
        ...
        while self.open_nodes:
            ...
            for obs_idx in range(len(node.safe_post)):
                ...
                obs_child_list_idx = []
                # observation actions
                for node_action_idx, node_action in enumerate(self.model.action_feasibility(node.safe_post[obs_idx])):
                    ...
                    risk = self.model.calc_risk(safe_prior, new_node.action_idx)
                    new_node.risk = risk * new_node.parent_risk_prob_safe
                    new_node.risk_u = min(risk+self.risk_error_range/2, 1.) * new_node.parent_risk_prob_safe
                    new_node.risk_l = max(risk-self.risk_error_range/2, 0.) * new_node.parent_risk_prob_safe
    ...
    def gurobi_solver_LP(self):
        try:
            ...
            gurobi_var = m.addVars((len(self.BFS_tree)), lb=0.0, ub=1.0, obj=1.0, vtype=GRB.SEMICONT)
        ...
        if self.integral_sol:
            ...
            #for _ in range(k):
            while not self.feasible or iteration < self.rounding_min_iteration:  # and iteration<10000:
                iteration += 1
    ...
    def gurobi_solver_ILP(self):
        try:
            ...
            # risk constraint
            if self.robust:
                risk_quicksum = quicksum(gurobi_var[i] * node.risk_l for i, node in enumerate(self.BFS_tree))
            else:
                #risk_quicksum = quicksum(gurobi_var[i] * (node.risk + (node.risk_u - node.risk)* self.risk_error_probability) for i, node in enumerate(self.BFS_tree))
                risk_quicksum = quicksum(gurobi_var[i] * (node.risk_l + (node.risk_u - node.risk_l) * (1-self.risk_error_probability)) for i, node in enumerate(self.BFS_tree))
                #risk_quicksum = quicksum(gurobi_var[i] * node.risk for i, node in enumerate(self.BFS_tree))
            m.addConstr(risk_quicksum <= self.model.cc)
            ...
            #gamma_percent = 0.3
            if self.robust:
                #gamma = int(math.ceil((1-self.risk_error_probability) * self.model.s_horizon))
                comb_list = list(itertools.combinations(range(self.model.s_horizon), self.gamma))
                for comb in comb_list:  # TODO: add risk_down, risk_up in each node for robust
                    m.addConstr(risk_quicksum + quicksum(gurobi_var[idx] * (self.BFS_tree[idx].risk_u-self.BFS_tree[idx].risk_l) for depth in comb for idx in range(self.depth_idx_list[depth], self.depth_idx_list[depth+1])) <= self.model.cc)
            ...
            for n in self.BFS_tree:
                if n.x_val == 1:
                    exp_risk += n.risk_l + (n.risk_u-n.risk_l)/2


            print(f'result: gamma:{self.gamma}, Obj:{self.obj_ILP}, E[risk]:{exp_risk}')
...
@njit
def randomized_rounding(gurobi_list_copy, cc, risk_list, reward_list, parent_list, child_list_idx_list, parent_act_idx_list, prune, prune_idx_list):
    risk_sum = 0.
    objective = 0.
    for index, var in enumerate(gurobi_list_copy):
        ...
        # Case: integral sol
            ...
            if prune:
                round_child_list = np.array([prune_idx_list[idx] for idx in round_child_list if idx != -1], np.int32)
